...
def stt_setup_kwargs(config: dict) -> dict:
    ...
    if backend == "qwen3asr_flash":
        import os as _os

        return {
            "api_key": stt.get("api_key") or _os.environ.get("DASHSCOPE_API_KEY", ""),
            "model": stt.get("model_name", "qwen3-asr-flash"),
            "language": stt.get("language", "zh"),
            "base_url": stt.get("base_url", "https://dashscope.aliyuncs.com/compatible-mode/v1"),
            "gen_kwargs": gen_kwargs,
        }
...
def tts_setup_kwargs(config: dict) -> dict:
    ...
    voxcpm：voices=personas 注册表全员（key=persona id），启动时预编码
    prompt cache，session.update 的 audio.output.voice 热切换；
    默认音色 = personas.default。
    gptsovits：走常驻推理服务（scripts/gptsovits_server.py），
    无需 personas 音色映射（服务端固定加载指定权重）。
    """
    tts = config["tts"]
    if tts.get("backend") not in ("voxcpm", "gptsovits"):
    ...
    if tts.get("backend") == "gptsovits":
        return {
            "server_url": tts.get("server_url", "http://127.0.0.1:8899"),
            "speed": float(tts.get("speed", 1.0)),
            "sample_rate": int(tts.get("sample_rate", 16000)),
            "blocksize": int(tts.get("blocksize", 512)),
            "bilingual": bool(tts.get("bilingual", False)),  # 只念「【日语】」段（译文只显示）
        }
