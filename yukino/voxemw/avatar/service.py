"""Avatar 积木：数字人服务（流式说话头）。

独立 GPU 进程。输入：参考肖像（persona 的 ref_image）+ 16kHz int16 PCM 音频流；
输出：JPEG 视频帧流（1280×720，25fps 节奏）。
引擎按 avatar.backend 选择（voxemw/avatar/factory.py）：
  - avtr1：AVTR-1 真人说话头（TensorRT，须在 pixi env 直调启动，
    勿 pixi run——会按 lock 重同步 env 覆盖 pip 降级；start_assistant.sh 已内置）
  - tha3：动漫参数化说话头（PyTorch，项目 .venv 直启即可）
两种引擎 ws 协议完全一致，本文件对引擎实现无感知。
...
def main() -> None:
    ...
    from voxemw.avatar.factory import build_engine

    engine = build_engine(avatar, image)
