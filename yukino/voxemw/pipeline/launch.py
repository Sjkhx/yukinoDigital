...
def _install_custom_handlers(config: dict) -> None:
    ...
    def get_stt_handler(module_kwargs, stop_event, queue_in, queue_out, speculative_turns, *kwargs):
        ...
        elif module_kwargs.stt == "qwen3asr_flash":
            from voxemw.pipeline.stt_qwen3asr_flash import Qwen3ASRFlashSTTHandler as stt_cls
        if stt_cls is None:
    ...
    def get_tts_handler(module_kwargs, stop_event, queue_in, queue_out, should_listen, *kwargs):
        backend = module_kwargs.tts
        if backend not in ("voxcpm", "gptsovits"):
        ...
        if backend == "voxcpm":
            ...
            handler_cls = VoxCPMTTSHandler
        else:
            from voxemw.pipeline.tts_gptsovits import GPTSovitsTTSHandler

            handler_cls = GPTSovitsTTSHandler
...
def _patch_torch_hub_offline_fallback() -> None:
    ...
    def _load_with_local_fallback(repo_or_dir, model, *args, **kwargs):
        ...
        if repo_or_dir == "snakers4/silero-vad" and local.is_dir():
...
def _patch_bilingual_transcript() -> None:
    """双语模式显示侧：LLM 输出「【日语】…【译文】…」时，
    ...
    def on_assistant_text(self, conn_id, event, **kwargs):
        ...
        for ev in events:
            if getattr(ev, "type", "") == "response.output_audio_transcript.done":
                ev.transcript = ja_d  # 聊天框正文只显示日语
            new_events.append(ev)
        if zh_d:
            new_events.append(TranslationDeltaEvent(delta=zh_d))
        return new_events

    def finish_response(self, conn_id, status="completed", reason=None, **kwargs):
        # 上游签名 (conn_id, status, reason)——必须完整透传，缺参数会让
